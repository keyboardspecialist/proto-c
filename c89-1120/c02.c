/* C compiler (last1120), pass 1 part 3: external definitions, statements.
   last1120 globals are untyped (int words): `name`, `name const`, `name[n] ...`,
   or `name() body`. Array locals are reassignable pointers (+ storage for sized
   ones), which is the `auto x[]` idiom. Control flow is emitted as structured
   wasm (see cc.h, wat.c). */
