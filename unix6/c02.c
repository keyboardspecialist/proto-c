...
/* A switch case must be a direct child of the switch body: the WAT backend
 * dispatches cases by splitting the body into segments, so a case nested in a
 * loop or if emits a structured wasm block the dispatch cannot branch into
 * (irreducible control flow -- e.g. Duff's device). cfdepth counts if/loop
 * bodies entered; swcfdepth is its value at the current switch, so a case
 * deeper than that is rejected rather than silently miscompiled. */
static int cfdepth;
static int swcfdepth = -1;
static int swcaseerr;			/* reported once per switch */

static void casecheck(void)
{
	if (swcfdepth >= 0 && cfdepth > swcfdepth && !swcaseerr) {
		error("case inside a loop or if in switch is not supported");
		swcaseerr = 1;
	}
}
...
stmt:
	...
	case 19:
		...
		case 15:
			o1 = brklab;
			brklab = isn++;
			o2 = swcfdepth;		/* cases must sit at this control-flow depth */
			swcfdepth = cfdepth;
			swcaseerr = 0;
			np = (word *) pexpr();
			sw_begin(np, brklab);
	...
	if (dbg) {
		...
		while (hl--) {
			if (cs[4] && cs[0] == 5 && g_ndbgvar < 64) {
				...
				g_dbgvar_size[g_ndbgvar] = trlength(cs);
				g_ndbgvar++;
			}
			cs = cs + pssiz;
		}
