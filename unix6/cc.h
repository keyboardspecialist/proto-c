...
/* for-loop step capture (the step runs after the body, so emit it now and
 * replay the captured text past the body). */
void cg_cap_begin();
char *cg_cap_end();		/* returns char* -- must be declared on 64-bit */

/* preprocessor (§12): expand a just-read identifier if it names a #define
 * macro (splices its text onto the input). Returns 1 if it was a macro. */
