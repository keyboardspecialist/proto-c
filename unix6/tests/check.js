...
const DIR = __dirname;
const CFRONT = path.join(DIR, '..', 'cfront-unix6');
...
// 1975 additions over prestruct-c: for, sizeof, #define, #include
e = build('forloop').exports;
ok(e.forsum(10) === 45 && e.forbrk() === 7, 'for statement (sum 0..9=45, for(;;)+break=7)');

e = build('szof').exports;
ok([e.szi(), e.szc(), e.szd(), e.szp(), e.szs(), e.szv()].join(',') === '4,1,8,4,8,40',
	'sizeof int/char/double/ptr/struct/array = 4,1,8,4,8,40');

e = build('ppdef').exports;
ok(e.ppx() === 12, '#define token replacement (N*DBL = 6*2)');

// #include resolves the header relative to the compiler's cwd, so run from DIR.
{
	const wat = path.join(DIR, 'ppinc.wat'), wasm = path.join(DIR, 'ppinc.wasm');
	fs.writeFileSync(wat, cp.execSync(`${CFRONT} ppinc.c`, { cwd: DIR }));
	cp.execSync(`${WAT2WASM} ${wat} -o ${wasm}`);
	const inc = new WebAssembly.Instance(
		new WebAssembly.Module(fs.readFileSync(wasm)), {}).exports;
	ok(inc.incx() === 81 && inc.sqr(5) === 25, '#include splices header (sqr(K)=9*9, sqr(5)=25)');
}
