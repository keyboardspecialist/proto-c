...
/* Per-statement debug hook: set $__line, then call $__break(line, fp) only when
 * the runtime has armed $__bp_armed. No-op unless compiled with -g. */
...
static int ischar(word *p) { return (p[1] & 07) == 1 && (p[1] & 070) == 0; }

/* a scalar float/double type (not a pointer to one) */
static int isftype(int ty) { return (ty & 070) == 0 && ((ty & 07) == 2 || (ty & 07) == 3); }
...
word *p;
{
	...
	case 32: case 33:	/* post ++ / -- */
	{
		...
		int post = (op >= 32);
		int delta = (int) p[4];		/* element-size step (plength) */
		int lf = tflt(p);
 ...
 * In this pre-&&/|| dialect, & and | are LOGICAL short-circuit operators in
 * boolean context (the original cbranch in c11.c did this), so `if (1 & 2)` is
 * true, not bitwise 0. Value contexts (gexpr) keep them bitwise. */
void gcond(p)
word *p;
{
	int op;
	...
	op = (int) p[0];
	switch (op) {
	case 47:		/* & : logical and (short-circuit) */
		gcond((word *) p[3]);
	...
	case 48:		/* | : logical or (short-circuit) */
		gcond((word *) p[3]);
