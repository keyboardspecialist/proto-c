...
char **argv;
{
	...
	init("extern", 6);
	init("static", 7);
	init("register", 5);	/* 1975: a storage class; treated as auto */
	init("entry", 9);	/* 1975: reserved, not implemented by any compiler */
	init("goto", 10);
	init("return", 11);
	init("if", 12);
	init("while", 13);
	...
	init("do", 19);
	init("default", 20);
	init("for", 22);	/* 1975: for ( e1 ; e2 ; e3 ) statement */
	init("sizeof", 25);	/* 1975: sizeof unary operator -> int constant */
	xdflg = 0;
	watmodopen();
	while (!eof) {
		extdef();
...
com1:
	...
	case 123:	/* letter */
	{
		...
		while (k < NAMSIZ)
			id[k++] = '\0';
		peekc = c;
		if (pp_expand(id)) {	/* a #define macro: splice its text, re-lex */
			c = getchar();	/* pull the first char of the spliced text */
			goto loop;
		}
		for (k = 0; k < NWPS; k++)
			symbuf[k] = 0;
...
/* sizeof (1975, §7.2.10): yields the byte size of its operand as an int
 * constant. Operand forms: a type-name -- int/char/float/double/struct tag,
 * optionally followed by '*' (a pointer is always 4 bytes here) -- or a
 * declared object, whose size comes from the symbol table (whole array for an
 * array). The operand may be parenthesized. */
int dosizeof()
{
	int o, paren, isptr, base;
	word *sym;
	...
	if (o == 19 && cval >= 0 && cval <= 4) {	/* a type-name */
		base = cval;
		if (base == 4) {		/* struct tag */
			if (symbol() != 20 || csym[0] != 8) {
				error("sizeof: bad structure type");
				return (0);
			}
			sym = csym;
		}
		while ((o = symbol()) == 42)	/* '*' -> pointer */
			isptr = 1;
		peeksym = o;
		if (paren && symbol() != 7)	/* ) */
		...
		case 2: return (8);		/* float (modeled as f64) */
		case 3: return (8);		/* double */
		case 4: return ((int) sym[3]);	/* struct: stored size */
		}
		return (0);
	}
	if (o == 20) {				/* a declared object */
		sym = csym;
		if (paren && symbol() != 7)	/* ) */
			error("sizeof: missing )");
		if ((sym[1] & 030) == 030)	/* array: whole-array size */
			return (tlength(sym));
		return (length(sym));
	}
	error("sizeof: bad operand");
	return (0);
}
...
tand:
	...
	/* sizeof (1975): a keyword that yields an int constant */
	case 19:
		if (cval != 25 || andflg)	/* only sizeof; needs an operand slot */
			goto syntax;
		*cp++ = block(1, 21, 0, 0, (word) dosizeof());
		goto tand;
